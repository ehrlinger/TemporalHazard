...
- **A `SETG3` entry refusal under `WEIBULL` is recorded once
  ...
  now has one row, and its phase keeps the values written. The job still
  warns with the same code; the warning now names the late shape and its
  fixed parameters (`GAMMA=1 ALPHA=1 ETA=1 fixed:tau`), as the same job
  without `WEIBULL` already did, rather than `TAU` alone.
