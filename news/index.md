...
- **[`hzr_translate_sas()`](https://ehrlinger.github.io/TemporalHazard/reference/hzr_translate_sas.md)
  ...
  accepts) or could not use (a `MUE` or `MUL` with no shape operand) are
  recorded in `$untranslated`, but the fit chunk used to be emitted
  anyway, as `hazard(fit = TRUE, theta = c())` under the default
