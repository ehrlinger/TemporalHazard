...
- **[`hzr_translate_sas()`](https://ehrlinger.github.io/TemporalHazard/reference/hzr_translate_sas.md)
  ...
  A `SELECTION NOSTEPWISE` (or `NOSW`) job is now refused like every
  other `SELECTION` job. It was read as no screen at all and translated
  to a plain fit with every candidate in the model, but `PROC HAZARD`
  still screens, forward only, with each candidate starting out of the
  model. And a phase variable that is not in the fitted model (an `/E`
  variable, or a covariate of a phase the job does not select) still
  deletes its missing rows in `PROC HAZARD`, which
  [`hazard()`](https://ehrlinger.github.io/TemporalHazard/reference/hazard.md)
