...
#>      3 31      1     2.53        0.0323        0.0816 0.9260  0.3360
#>      4 30      2     3.49        0.0667        0.1160 0.6350  0.4250
#>      5 31      7     4.58        0.2260        0.1480 1.2800  0.2570
#>      6 30     10     5.67        0.3330        0.1890 3.3100  0.0689
#>      7 31      8     7.59        0.2580        0.2450 0.0218  0.8830
#>      8 30     14     8.72        0.4670        0.2910 3.2000  0.0735
#>      9 31     10    13.50        0.3230        0.4350 0.8960  0.3440
#>     10 30     15    18.90        0.5000        0.6320 0.8230  0.3640
...
#>         nu 20 100  0.2443 0.0176  0.2176 0.2867   0.2196   0.2799
#>        age 20 100 -0.0028 0.0016 -0.0070 0.0002  -0.0061  -0.0003
#>     status 20 100  0.7053 0.2266  0.3261 1.0926   0.3525   1.0779
#>        mal 20 100  0.4601 0.3280 -0.2047 0.9677  -0.1301   0.9321
#>     com_iv 20 100  0.7384 0.3397  0.0757 1.3481   0.1426   1.3177
```
...
- `EARLY` and `CONSTANT` become the `formula` of the matching
  [`hzr_phase()`](https://ehrlinger.github.io/TemporalHazard/reference/hzr_phase.md).
- `CONSERVE` goes into `control` as `conserve = TRUE`. `CONDITION=14`
  and `QUASI` have no
  [`hazard()`](https://ehrlinger.github.io/TemporalHazard/reference/hazard.md)
  equivalent, so nothing is written for them: `CONDITION=` stops
...
`CONSERVE` becomes `control$conserve = TRUE`, and the `PARMS`
early-phase shape (`MUE`/`THALF`/`NU`/`M FIXM`) becomes a
`hzr_phase("cdf", ...)` with `m` fixed. `FIXM` holds `m` only through
`fixed = "m"` on the phase. `QUASI` and `CONDITION=14` are not in
`control`, because
[`hazard()`](https://ehrlinger.github.io/TemporalHazard/reference/hazard.md)
reads neither: they are listed in `job$untranslated` with the reason.
`QUASI` chooses `PROC HAZARD`‘s optimizer, and
[`hazard()`](https://ehrlinger.github.io/TemporalHazard/reference/hazard.md)
has no choice to make: it always uses BFGS (a multiphase fit may run a
Nelder-Mead warm-up first, and a stop that fails SAS’s gradient test
continues with [`stats::nlm()`](https://rdrr.io/r/stats/nlm.html)).
`CONDITION=` stops `PROC HAZARD`’s optimizer as ill-conditioned once its
Hessian approximation’s condition estimate passes the limit, and
[`hazard()`](https://ehrlinger.github.io/TemporalHazard/reference/hazard.md)
has no such stop; it warns about the final Hessian instead. Earlier
versions of the translator wrote both into `control`, where nothing read
them. `dist = "multiphase"` is emitted whenever `phases` is, and `theta`
carries the full interleaved multiphase start vector (early scale, then
every early-phase shape parameter in phase order, then the constant/late
scale), not just the phases’ log(mu) starts.
