...
test run there, and only there. The other four platforms still skip
them. Moving the pin is a deliberate PR, because the corpus test’s
floors are measured against that commit. A step fails the job if the
files those tests read are missing, so a moved path fails CI instead of
turning back into silent skips.
