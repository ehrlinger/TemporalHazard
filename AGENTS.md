...
Unlike ggRandomForests, this repo has **no** `.claude/house-style.md`; the voice lives in the
vault and is applied through the `ehrlinger-writing` skill.
...
follows a specific voice, reader persona and project context. The default persona for this
package is the **public CRAN user** — no institutional context, reading `?fn` or a vignette —
except in `sas-to-r-migration.qmd`, which is written for the **bilingual SAS-to-R migrant**.
