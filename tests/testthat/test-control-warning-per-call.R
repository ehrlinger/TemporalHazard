...
#
# Since #405 every unread `control` element draws a warning and hazard()
...
# a three-step screen and scaled with replicates in a bootstrap. The result
# is unaffected; the harm is to OTHER warnings. Past 50, R prints only
...
test_that("a screen warns once, not once per candidate refit (#410)", {
  ...
  # And nothing else was muffled on the way: the other warnings are the
  # same ones the plain screen raises.
  expect_identical(sort(got$other), sort(plain$other))
})

test_that("a bootstrap warns once, not once per replicate (#410)", {
  ...
  expect_identical(got$value$n_success, plain$value$n_success)
  expect_identical(got$value$summary$parameter, plain$value$summary$parameter)
})
