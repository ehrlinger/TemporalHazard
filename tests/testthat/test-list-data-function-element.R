...
# function survives, and the formula path reached it. Every case here runs at
# n = 1, 2 and 40 so that the parameter which hid it is pinned, not sampled.
...
test_that("an element no expression can look up is not refused", {
  ...
  # this fix made one.
  d <- ldf_setup_unnamed <- list(rep(2.5, 3), rep(1L, 3), base::rep)
