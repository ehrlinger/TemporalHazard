...
- `CONSERVE` goes into `control` as `conserve = TRUE`. `CONDITION=14`
  ...
  optimizer. `hazard()` offers no optimizer choice (it uses BFGS, or
  L-BFGS-B when parameters are bounded) and has no such stop.
...
optimizer, and `hazard()` has no choice to make: it uses BFGS, or L-BFGS-B
when parameters are bounded (a multiphase fit may run a Nelder-Mead warm-up
first, and a stop that fails SAS's gradient test continues with
`stats::nlm()`). `CONDITION=` stops
