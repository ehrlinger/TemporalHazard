...
- **A job `PROC HAZARD` itself refuses, or one whose model this
  ...
  as a number, an unknown keyword, a template's `?`), a `SETG3` refusal,
  `FIXMNU1`, a non-zero `DELTA`, `FIXTAU` with no `TAU` written, and
  `FIXGE2`/`FIXGAE2` without `WEIBULL` each emit a `stop()` naming the
  ...
  Operands carrying a macro reference are left undecided, because SAS
  expands them before `PROC HAZARD` reads the statement.
