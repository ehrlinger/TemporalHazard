...
- **A job `PROC HAZARD` itself refuses, or one whose model this
  ...
  One case still halts, for want of anything to emit: where the value
  `PROC HAZARD` refuses is also one `hzr_phase()` will not build, there
  is no fit to warn above, so the job stops. Delete that job from the
  file if you need the others' results before you can correct it.
