...
`start_seed` for a multiphase one. `hazard()` refuses any other
element rather than ignoring it, so a mistyped name is an error, not a
silently unchanged fit.
...
- `CONSERVE` becomes `control = list(conserve = TRUE)`. `CONDITION=14`
  ...
  through `rcond` rather than taking a tolerance. `hazard()` refuses a
  `control` element it would not use, so they are left out.
