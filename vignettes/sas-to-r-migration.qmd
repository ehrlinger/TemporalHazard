...
- **A `SELECTION` statement is translated into a real screen, and the
  ...
  approximate variances while selecting. The entry statistic here
  reproduces that approximation (except for a candidate refitted because
  its information is indefinite), but the Wald tests behind each removal
  use the full Hessian, so removals can differ; the emitted document says
  so in a callout above the chunk. Compare the
