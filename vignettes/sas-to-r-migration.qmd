...
- **A `SELECTION` statement is translated into a real screen, and the
  ...
  approximate variances while selecting and this package uses the full
  Hessian, so the statistics driving each enter and drop decision differ;
  the emitted document says so in a callout above the chunk. Compare the
