...
- `EARLY` and `CONSTANT` become the `formula` of the matching
  `hzr_phase()`.
- `CONSERVE` becomes `control = list(conserve = TRUE)`. `CONDITION=14`
  and `QUASI` have no `control` equivalent: `hazard()` already optimizes
  by quasi-Newton (BFGS), and it reports the Hessian's conditioning
  through `rcond` rather than taking a tolerance. Passed as
  `control = list(condition = 14, method = "bfgs")` they would only draw a
  warning that they do nothing, so they are left out.
...
read `control$method`, and the fitting path uses BFGS regardless (a
multiphase fit may run a Nelder-Mead warm-up first, and a stop that fails
SAS's gradient test continues with `stats::nlm()`), so the entry only
