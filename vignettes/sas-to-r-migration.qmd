...
`control` holds only settings the fit reads: `maxit`, `reltol` and
`abstol` for every model, plus `n_starts`, `conserve`, `phase_share_tol`
and `start_seed` for a multiphase one. `hazard()` refuses any other
