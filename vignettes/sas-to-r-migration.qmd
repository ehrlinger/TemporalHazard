...
- **A `SELECTION` statement is translated into a real screen, and the
  ...
  `stop()`: `FAST`, `MAXVARS`, `RESTRICT`, `ROBUST`, `SEMIROBUST`, a
  per-variable `MOVE=` or `ORDER=`, and a variable held by `/I` in one
  phase but movable in another. On the reference corpus one job in four
  translates, `ROBUST` accounting for most of the rest.
